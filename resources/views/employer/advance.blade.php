...
<div  ng-controller="EmployerManagerController" ng-init="load({{$empid}})">
	...
	<div class="emp-section" id="emp-assistant">
		...
		<table class="table table-responsive table-hover table-bordered table-angular">
			...
			<tbody>
				<tr dir-paginate="ass in assis|orderBy:sortType:sortReverse|filter:search|filter: (flagStatus || '') && {status:filterStatus}|itemsPerPage:showitems" pagination-id="assis" id="content-table-admin">
					<td><%ass.id%></td>
					<td>
						<span><%ass.name%></span>
					</td>
				...
				<tr dir-paginate="post in posts|orderBy:sortTypePost:sortReversePost|filter:searchPost|filter: (flagStatusPost || '') &&  {status:filterStatusPost}|itemsPerPage:showitemsPost" pagination-id="posts" id="content-table-admin">
					...
					<td>
						<span ng-if="post.status==10">
							<button ng-click="confirmPost(post.id)" class="btn btn-sm btn-success">Confirm</button>
							<button ng-click="denyPost(post.id)" class="btn btn-sm btn-danger">Deny</button>
						</span>
					</td>
